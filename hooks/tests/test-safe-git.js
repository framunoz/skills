 ...
 * Uses Node.js directly to avoid bash escaping hell with JSON.
 */
...
const testCases = [
    {
        ...
        input: { tool_name: "Bash", tool_input: { command: "git commit -m 'feat: no backticks'" } },
        validate: (out) => out.decision === "allow" && !out.tool_input
    },
    {
        name: "2. Backticks in double quotes (must escape)",
        input: { tool_name: "Bash", tool_input: { command: 'git commit -m "feat: use `ls`"' } },
    ...
    },
    {
        ...
        input: { tool_name: "Bash", tool_input: { command: 'git commit -m"feat: `code`"' } },
    ...
    },
    {
        ...
        input: { tool_name: "Bash", tool_input: { command: 'git commit -m "feat: \\\\`code` Resort to \\\\\\`code\\`"' } },
        validate: (out) => out.tool_input?.command.includes('\\\\\\`code\\`')
    },
    {
        name: "7. Using --message= syntax (must escape)",
        input: { tool_name: "Bash", tool_input: { command: 'git commit --message="feat: `code`"' } },
        validate: (out) => out.tool_input?.command === 'git commit --message="feat: \\`code\\`"'
    },
    {
        name: "8. Apostrophes inside double quotes (Bug Fix Check)",
        input: { tool_name: "Bash", tool_input: { command: 'git commit -m "I\'m using `ls` and it\'s great"' } },
    ...
    },
    {
        ...
        input: { tool_name: "Bash", tool_input: { command: 'git commit -m "feat: `ls` \n more lines"' } },
        validate: (out) => out.tool_input?.command.includes('\\`ls\\`')
    },
    {
        name: "11. Multiple -m flags (escape all)",
        input: { tool_name: "Bash", tool_input: { command: 'git commit -m "subject `s`" -m "body `b`"' } },
    ...
    },
    {
        ...
        input: { tool_name: "Bash", tool_input: { command: 'git commit -m "feat: `ls` & $(whoami) | rm -rf /"' } },
        validate: (out) => out.tool_input?.command.includes('\\`ls\\`')
    },
    {
        name: "13. Backticks at start and end",
        input: { tool_name: "Bash", tool_input: { command: 'git commit -m "`start` middle `end`"' } },
    ...
    },
    {
        ...
        input: { tool_name: "Bash", tool_input: { command: 'git commit -m "feat: ``"' } },
        validate: (out) => out.tool_input?.command === 'git commit -m "feat: \\`\\`"'
    },
    {
        name: "15. Mixed quote types and backticks (Extreme case)",
        input: { tool_name: "Bash", tool_input: { command: "git commit -m 'Double \" within single and `backtick`'" } },
        validate: (out) => out.tool_input?.command.includes('\\`backtick\\`')
    },
    {
        name: "16. Backslash-backtick sequence (already escaped)",
        input: { tool_name: "Bash", tool_input: { command: 'git commit -m "feat: \\`already escaped\\`"' } },
        validate: (out) => out.decision === "allow" && !out.tool_input
    },
    {
        name: "17. Markdown code blocks inside message",
        input: { tool_name: "Bash", tool_input: { command: 'git commit -m "Here is some code:\n```javascript\nconst x = 1;\n```"' } },
        validate: (out) => {
    ...
    },
    {
        ...
        input: { tool_name: "Bash", tool_input: { command: 'git commit -am "hotfix: `urgent`"' } },
        validate: (out) => out.tool_input?.command.includes('\\`urgent\\`')
    },
    {
        name: "19. Environment variable prefix",
        input: { tool_name: "Bash", tool_input: { command: 'LANG=en_US.UTF-8 git commit -m "feat: `ls`"' } },
        validate: (out) => out.tool_input?.command.includes('\\`ls\\`')
    },
    {
        name: "20. Command chaining (Security check)",
        input: { tool_name: "Bash", tool_input: { command: 'git commit -m "done"; ls `pwd`' } },
        validate: (out) => out.tool_input?.command.includes('\\`pwd\\`')
    },
    {
        name: "21. False positive check (git commit in echo)",
        input: { tool_name: "Bash", tool_input: { command: 'echo "git commit -m some-msg"; ls `pwd`' } },
        validate: (out) => out.tool_input?.command.includes('\\`pwd\\`')
    },
    {
        name: "22. Nested backticks in message",
        input: { tool_name: "Bash", tool_input: { command: 'git commit -m "msg `ls \\`pwd\\``"' } },
        validate: (out) => {
    ...
    },
    {
        ...
        input: { tool_name: "Bash", tool_input: { command: 'git commit -m "don`t do this"' } },
        validate: (out) => out.tool_input?.command.includes('don\\`t')
    }
];

console.log("🧪 Running Robust Tests for safe-git-commit.js...");
let failed = 0;
...
if (failed > 0) {
    ...
    console.log("\n✨ All robust tests for safe-git-commit.js passed!");
}
