# Entry Schemas — Logbook

Logbooks are neutral containers: any entry type may coexist in any logbook. The `type` field on each entry determines which schema applies. `meta.json` has no declared type.
...
\* At least one of `went_well` or `went_wrong` must be non-empty. Both empty → rejected (exit 11). Missing sections are rendered as "No observations" — never fabricated.
...
Allowed in any logbook. Amendments append a correction record; `format.py` renders it as a callout beneath the original entry.

| Field | Type | Required | Notes |
|---|---|---|---|
| `amends` | `{id: int, ulid: string}` | yes | Both values must match an existing entry in the same logbook. `push.py` verifies before writing. |
...
`amends.id` or `amends.ulid` not found → rejected (exit 13).
