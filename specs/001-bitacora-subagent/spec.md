...
- Q: ¿Esquema por bitácora: fijo o mixto? → A: **Mixto (revisado 2026-04-19, sin `schema_type`)**. Las bitácoras son contenedores neutros sin tipo declarado. Cada entrada lleva su propio tipo (`tests`, `collaboration`, `free`). `meta.json` no almacena `schema_type` — la bitácora no restringe qué tipos de entrada puede recibir. El subagente infiere el tipo desde el contenido del mensaje y confirma cuando es ambiguo.
...
- **FR-002a**: Las operaciones de **escritura** (push de entradas) son exclusivas del subagente de bitácora. El agente principal, otros subagentes, y el usuario mediante comandos directos NO pueden escribir entradas. No existe una skill de push de acceso directo.
- **FR-002b**: La **creación de bitácoras** (`logbook-init`) es igualmente exclusiva del subagente. El usuario no invoca `logbook-init` directamente. La creación es transparente: cuando el subagente recibe una solicitud de push para un slug inexistente, crea automáticamente la bitácora (confirmando con el usuario antes si el slug no fue nombrado explícitamente).
...
- **FR-003a**: El subagente DEBE soportar una sintaxis de invocación abreviada: `@logbook <slug>: <mensaje>`. En este modo, el slug identifica la bitácora objetivo y el tipo de entrada se infiere del contenido de `<mensaje>` (FR-004a). Si la bitácora no existe, el subagente la crea automáticamente (previo aviso al usuario).
- **FR-003b**: El subagente DEBE enriquecer las entradas con contexto relevante de la conversación activa cuando esté disponible (archivos abiertos, código reciente, decisiones tomadas en la sesión). El enriquecimiento es complementario al dictado del usuario — no lo reemplaza — y se aplica a campos como `context`, `ai_contribution`, `human_contribution` según corresponda. Si el subagente añade contenido inferido del contexto, DEBE indicarlo claramente al usuario antes de persistir.
...
- **FR-007c**: DEBE existir una **skill de formateo** (`logbook-format`) invocable directamente por el usuario o por cualquier agente de Claude Code, además de por el propio subagente. La operación es idempotente y no altera datos — solo renderiza `entries.jsonl` a `rendered.md`. La descripción de la skill DEBE ser concisa y dirigida, incluyendo un hint para especificar el slug de la bitácora a renderizar.
...
- **Skill `logbook-format`**: skill invocable directamente por usuario y agentes. Idempotente. Renderiza `entries.jsonl` a `rendered.md` sin modificar datos de origen.
