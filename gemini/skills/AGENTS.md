...
When creating a new Gemini CLI skill manually, you must save the skill directory in one of the following locations depending on the requested scope:

| Scope | Path |
|----------|------|
| Project | `.gemini/skills/<skill-name>/` |
| Global | `~/.gemini/skills/<skill-name>/` |
| Agent-compatible (project) | `.agents/skills/<skill-name>/` |
| Agent-compatible (global) | `~/.agents/skills/<skill-name>/` |

**Constraint:** The directory name MUST match the `name` field in the frontmatter exactly.
